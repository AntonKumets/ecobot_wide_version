...
def send_welcome(message):
    ...
    bot.send_message(message.chat.id, 'Напиши /global_warming, ты и узнаешь как переработка мусора связана с замедлением глобального потепления')
    bot.send_message(message.chat.id, 'Напиши команду /recycling, и ты узнаешь как правильно сортировать отходы')
    bot.send_message(message.chat.id, 'Если ты не знаешь, что перед тобой, в любой момент загружай фото, чтобы по нему определить класс переробатываемого мусора. Есть 4 класса: макулатура, батарейки, пластиковые бутылки, стекло. Если фотографий несколько, загружай их по одной')
    bot.send_message(message.chat.id, 'После пользования нашим ботом, опишите свои впечатления в письме нам на почту: kumetsanton@gmail.com. Мы будем ждать ☺️')
